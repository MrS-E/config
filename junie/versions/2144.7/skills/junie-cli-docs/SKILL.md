...
* `additionalContext` on stdout — agent-facing. `UserPromptSubmit` prepends it to the prompt; `Stop` folds it into the agent's observer message on block / hard-halt retries **and** on a plain successful exit (Claude-style continue, see [Stop hook blocking and retries](#stop-hook-blocking-and-retries)). `SessionStart`, `SessionEnd`, and `PermissionRequest` ignore it.
