...
| `--check-only` | Report system information and the install configuration, then exit |
...
| `/review [prompt]`       | Start a [local code review](Junie-CLI.md#local-code-review) of your changes (compared to `main`, the last commit, or unstaged changes) before you commit them. If provided, `[prompt]` adds custom instructions to the selected review target. Requires a git repository.                                                                                  |
...
* **`demo.mp4`**: the screen recording, if the recorder started and stopped cleanly.
* **`captions.vtt`**: a WebVTT caption track based on screenshot timestamps.
